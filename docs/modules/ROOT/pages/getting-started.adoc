...
[TIP]
====
The `r` menu entry configures *report destinations* — a folder, an
S3-compatible bucket, and/or MongoDB — and live-probes each one (a write test)
as you answer, the same way the setup wizard probes hosts. Skip it and every
audit still saves its JSON report to a per-user default folder. See the
xref:configuration.adoc#_reporting_where_audit_reports_are_delivered[`reporting`
configuration] and xref:automation.adoc[Automation].
====
